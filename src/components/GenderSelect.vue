<template>
  <div class="gender-select">
    ...
    <!-- Existing Users -->
    <div v-if="existingProfiles.length > 0" class="existing-users">
      <p class="existing-label">{{ t.userMenu?.switchUser || 'Switch User' }}</p>
      <div class="user-list">
        <div
          ...
          @click="switchToUser(profile.id)"
        >
          <img
            :src="profile.gender === 'prince' ? `${baseUrl}math_prince.png` : `${baseUrl}math_princess.png`"
            class="user-avatar"
          />
          <span class="user-name">{{ profile.name || (profile.gender === 'prince' ? t.welcome?.defaultPrince : t.welcome?.defaultPrincess) }}</span>
        </div>
      </div>
      <div class="divider">
        <span>{{ t.userMenu?.addUser || 'New User' }}</span>
      </div>
    </div>
...
const existingProfiles = computed(() => profilesStore.profiles || [])
...
function switchToUser(profileId) {
  playClickSound()
